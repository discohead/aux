...
struct RewindHandlerTests {
    ...
    @Test func callsGetNowPlayingAndSeek() async throws {
        ...
        try await RewindHandler.handle(services: container, options: GlobalOptions(), seconds: 10.0, writer: writer)
    ...
    @Test func calculatesCorrectPosition() async throws {
        ...
        #expect(message.contains("20.0"))
    }

    @Test func clampsToZero() async throws {
        let container = ServiceContainer.mock()
        let mock = container.appleScript as! MockAppleScriptBridge
        mock.getNowPlayingResult = .success(.fixture(positionSeconds: 5.0))

        var capturedData: Data?
        let writer = JSONOutputWriter(destination: { capturedData = $0 })

        try await RewindHandler.handle(services: container, options: GlobalOptions(), seconds: 20.0, writer: writer)
